...
<template>
  ...
  <div class="mind-box">
    ...
    <el-drawer
      ...
      <el-button @click="changebg" size="small" type="primary">确定上传</el-button>
    </el-drawer>
    <!-- 左侧 -->
      ...
      <div class="mind-top clearfix">
        ...
        <div class="fr">
          <!-- <a href="javascript:void(0)" @click="showJsonData" class="svg-a-sm">
            <i class="el-icon-tickets">批量导入图片</i>
          </a> -->
          <a href="javascript:void(0)" @click="drawer = true" class="svg-a-sm">
            <i class="el-icon-tickets">批量上传图片</i>
          </a>
          <a href="javascript:void(0)" @click="showJsonData" class="svg-a-sm">
...
export default {
  ...
  methods: {
      ...
      kgBuilderApi.getDomainGraph(data).then(result => {
        if (result.code == 200) {
          if (result.data != null) {
            ...
            this.nodelistone=_this.graphData.nodes;
          }
        }
      });
    },
    ...
    changebg(){
      //对节点进行遍历
      for(var i=0;i<this.nodelistone.length;i++){
        var flag=0;
/*        console.log(parseInt(this.nodelistone[i].uuid/10));
        console.log(this.nodelistone[i].uuid);
        console.log(this.nodelistone[i].name);*/
        var data;
        for(var j=0;j<this.nodeImageList.length;j++){
          if(this.nodelistone[i].name==this.nodeImageList[j].name.slice(0, -4)){
            data = {
              domainId: parseInt(this.domainId),
              nodeId:this.nodelistone[i].uuid,
              //imageList: JSON.stringify(this.nodeImageList)
              imagePath: this.nodeImageList[j].file
            };
            flag=1;
            break
          }
    ...
    beforeUpload(){
      if(this.nodeImageList.length>this.imgmaxlength){
        this.$message({
          message: '最多'+this.imgmaxlength+'张图片',
          type: 'warning'
        });
      }
    },
    uploadSuccess(res, file) {
      if (res.success == 1) {
        for (let i = 0; i < res.results.length; i++) {
          ...
          if(this.nodeImageList.length<=this.imgmaxlength){
            this.nodeImageList.push({ file: fileItem.url,name:fileItem.name, imageType: 0 });
          }
        }
      } else {
